...
var persisters = (function () {
    ...
    var sessionKey = localStorage.getItem("kingsvaley-sessionkey");

    function saveUserData(userData) {
        localStorage.setItem("nickname", userData.nickname);
        localStorage.setItem("kingsvaley-sessionkey", userData.sessionKey);
    ...
    function clearUserData() {
        ...
        localStorage.removeItem("kingsvaley-sessionkey");
        nickname = "";
        sessionKey = "";
    }
    ...
    var UserPersister = Class.create({
        ...
        register: function (player, success, error) {
            var url = this.rootUrl + "players";
            var playerData = {
                Name: user.username,
                nickname: user.nickname,
                authCode: CryptoJS.SHA1(user.username + user.password).toString()
            };
            httpRequester.postJSON(url, playerData,
				function (data) {
				    saveUserData(data);
				    success(data);
				}, error);
