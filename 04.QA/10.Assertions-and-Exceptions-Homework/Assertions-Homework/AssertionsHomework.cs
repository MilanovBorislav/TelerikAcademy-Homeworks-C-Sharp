...
class AssertionsHomework
{
    public static void SelectionSort<T>(T[] arr) where T : IComparable<T>
    {
        ...
        for (int arrElement = 0; arrElement < arr.Length - 1; arrElement++)
        {
            Debug.Assert(arr[arrElement].CompareTo(arr[arrElement + 1]) < 0, "The array isn't sorted.");
        }
    }
        ...
        where T : IComparable<T>
    {
        ...
        {
            Debug.Assert(startIndex >= 0, "Start index must be positive!");
            Debug.Assert(endIndex >= startIndex, "End index must be bigger than startindex!");
